	...
	if (!pendingTransactions?.length) {
		return (
			<div className="px-6 sm:px-12 w-full">
				...
				<p className="text-lm-gray-800 dark:text-dm-gray-200 mt-4">{t('pagePending.noFound')}</p>
			</div>
		);
	}
		...
		<div className="px-6 sm:px-12 w-full">
			...
			<div className="py-2 w-full">
				<div className="space-y-2">
					{[...pendingTransactions].sort(reverse(compareBy((pt) => pt.created))).map((pt) => {
						...
						const cred = cfg
							? filterItemByLang(cfg.credential_metadata.display, 'locale')?.name
							: null;
						return (
							<div
