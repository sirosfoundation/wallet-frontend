...
export interface UseOID4VPFlowReturn {
	 ...
	 */
	handleAuthorizationRequest: (authorizationRequestUrl: URL) => Promise<OID4VPFlowResult>;
	/**
	...
	handleCredentialSelection: (
		verifierInfo: OID4VPVerifierInfo,
		dcqlQuery?: DcqlQuery.Input,
		preMatchedCredentials?: Map<
			string,
			{ credentials: number[]; requestedFields: Array<{ name?: string; path?: string[] }> }
		>,
	...
	sendDCAPIResponse: (
		...
		credentials: OID4VPSelectedCredential[],
	) => Promise<OID4VPFlowResult>;
}
...
export function useOID4VPFlow(options: UseOID4VPFlowOptions = {}): UseOID4VPFlowReturn {
	...
	const handleAuthorizationRequest = useCallback(
		async (authorizationRequestUrl: URL): Promise<OID4VPFlowResult> => {
			setIsLoading(true);
			setError(null);
			...
			try {
					...
					try {
						...
						return {
							...
							transactionData: result.parsedTransactionData?.map((td) => ({
								type: 'transaction',
								description:
									('description' in (td.parsed ?? {}) ? (td.parsed as any).description : null) ||
									JSON.stringify(td.parsed || td).slice(0, 100),
								data: td,
							})),
						};
					} catch (err) {
				...
				throw new OIDFlowError({
					code: 'NO_TRANSPORT',
					message: 'No transport available for verifiable presentation',
				});
			} catch (err) {
				const error =
					err instanceof OIDFlowError
						? err
						: new OIDFlowError({
								code: 'FLOW_ERROR',
	...
	const handleCredentialSelection = useCallback(
		async (
			...
			try {
				if (!options.onCredentialSelection)
					throw new OIDFlowError({
				...
				if (preMatchedCredentials) {
					...
					throw new OIDFlowError({
						code: 'NO_DCQL_QUERY_OR_PREMATCHED_CREDENTIALS',
						message: 'No dcqlQuery or preMatchedCredentials provided',
					});
				}
				...
				if (conformantCredentialsMap.size === 0) {
					throw new OIDFlowError({
						...
						message: 'No credentials available for selection',
					});
				}
				...
				for (const [descriptorId, batchId] of selectionMap.entries()) {
					...
					if (!walletState)
						throw new OIDFlowError({
							code: 'WALLET_STATE_UNAVAILABLE',
							message: 'Wallet state not available',
						});
					...
					selected.push({
						...
						disclosedClaims: dcqlQuery
							? (dcqlQuery.credentials
									.find((c) => c.id === descriptorId)
									?.claims?.map((c) => c.path?.join('.')) ?? [])
							: (preMatchedCredentials
									?.get(descriptorId)
									?.requestedFields?.map((f) => f.path?.join('.')) ?? []),
					});
				}
		...
		[keystore, options, waitForCredentials, onError],
	);
	...
	const sendAuthorizationResponse = useCallback(
		async (selectedCredentials: OID4VPSelectedCredential[]): Promise<OID4VPFlowResult> => {
			setIsLoading(true);
			setError(null);
			...
			try {
				...
				if (transportType === 'websocket' && transport) {
					...
					try {
						...
						if (keystore) {
							await recordPresentationHistory(
								keystore,
								api,
								selectedCredentials,
								verifierAudienceRef.current,
						...
						if (!result.success) {
							throw new OIDFlowError({
								code: result.error?.code || 'AUTHORIZATION_RESPONSE_FAILED',
								message: result.error?.message || 'Authorization response failed',
							});
						}
				...
				if (transportType === 'http_proxy' && openID4VP) {
					...
					if (result && 'presentation_during_issuance_session' in result) {
						return {
							success: true,
							responseData: {
								presentation_during_issuance_session: result.presentation_during_issuance_session,
							},
						};
					}
	 ...
	 * Handle DC API request - parse URL, match credentials, store session
	 */
	const handleDCAPIRequest = useCallback(
		async (request: DCAPIRequest, verifiedOrigin: string): Promise<OID4VPFlowResult> => {
			setIsLoading(true);
			try {
				...
				const conformantCredentials = buildConformantCredentialsMap(matches, request.dcqlQuery);
	 ...
	 * Send DC API response - sign, record history, send via session
	 */
	const sendDCAPIResponse = useCallback(
		async (
			session: DCAPISession,
			selectedCredentials: OID4VPSelectedCredential[],
		): Promise<OID4VPFlowResult> => {
...
async function recordPresentationHistory(
	...
	const presentations = await Promise.all(
		selectedCredentials.map(async (cred) => ({
			transactionId,
