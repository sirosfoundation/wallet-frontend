...
export const WalletCompanionProvider = ({ children }: { children: ReactNode }) => {
	const [api] = useState<WalletCompanionInterface | null>(() => window.WalletCompanion ?? null);
	...
	const register = useCallback(async () => {
		...
		const result = await api.registerWallet({
			...
			url: walletUrl,
			protocols: ['openid4vp-v1', 'openid4vp-v1-signed', 'openid4vp-v1-unsigned'],
		});
		if (result.success) setIsRegistered(true);
	}, [api, walletUrl]);

	const value = useMemo(
		() => (api && WALLET_COMPANION_INTEGRATION ? { api, isRegistered, isLoading, register } : null),
		[api, isRegistered, isLoading, register],
	);

	return (
		<WalletCompanionContext.Provider value={value}>{children}</WalletCompanionContext.Provider>
	);
};
