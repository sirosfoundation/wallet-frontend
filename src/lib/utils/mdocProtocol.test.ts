...
it.skip('can cbor encode a data structure', () => {
	const m = {
		version: '1.0',
		documents: [new Map([['docType', 'unit.test.doc.type']])],
		status: 0,
	};

	const encoded = cborEncode(m);
	// console.log(uint8ArraytoHexString(encoded));
	expect(uint8ArraytoHexString(encoded)).toBe(
		'b900036776657273696f6e63312e3069646f63756d656e747381a167646f635479706572756e69742e746573742e646f632e747970656673746174757300',
	);
});
...
it.skip('can encode, encrypt, decrypt and decode an mdoc response', async () => {
	...
	const decryptedText = (await decryptMessage(
		SKDevice,
		iv,
		sessionDataDecoded.get('data'),
		true,
	)) as Uint8Array;

	// const decryptedDecodedText = cborDecode(decryptedText);
	expect(uint8ArraytoHexString(mdocResponseEncoded)).toStrictEqual(
		uint8ArraytoHexString(decryptedText),
	);
