...
export const toHex = (u8?: Uint8Array | null) =>
	u8 ? [...u8].map((b) => b.toString(16).padStart(2, '0')).join('') : '';
...
export const parseOhttpKeys = (buf: ArrayBuffer): KeyConfig[] => {
	...
	while (off + 2 <= dv.byteLength) {
		const cfgLen = dv.getUint16(off, false);
		off += 2;
		if (off + cfgLen > dv.byteLength) break;
		const base = off;
		let p = 0;

		const keyId = dv.getUint8(base + p);
		p += 1;
		const kemId = dv.getUint16(base + p, false);
		p += 2;
		...
		const pubLen =
			kemId === 0x0020
				? 32
				: (() => {
						throw new Error(`Unsupported KEM 0x${kemId.toString(16)}`);
					})();
		// const pubLen = 32;
		const publicKey = new Uint8Array(buf, base + p, pubLen);
		p += pubLen;

		const algsLen = dv.getUint16(base + p, false);
		p += 2;
...
export const encryptedHttpRequest = async (
	relayUrl: string,
	keysInfo: HpkeConfig,
	...
	const hdr = concat(encode1(keyId), encode2(kemId), encode2(kdfId), encode2(aeadId));
	...
	let body;
	if (requestParams.headers && requestParams.headers['Content-Type'] === 'application/json') {
		// TODO: assuming json or forms only
		logger.debug('parsing as json');
		body = requestParams.body
			? new TextEncoder().encode(JSON.stringify(requestParams.body))
			: new TextEncoder().encode('{}');
	} else {
		logger.debug('parsing as form');
		body =
			typeof requestParams?.body === 'string'
				? new TextEncoder().encode(requestParams.body)
				: new Uint8Array(0);
	}
	...
	const requestAuthorization = (requestParams.headers as Record<string, string> | undefined)?.[
		'Authorization'
	];
