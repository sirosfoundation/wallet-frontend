...
export async function getSdJwtVcMetadata(
	credential: string,
): Promise<{ credentialMetadata: any } | { error: 'NOT_FOUND' }> {
	try {
		const credentialHeader = JSON.parse(
			new TextDecoder().decode(fromBase64(credential.split('.')[0] as string)),
		);
		const credentialPayload = JSON.parse(
			new TextDecoder().decode(fromBase64(credential.split('.')[1] as string)),
		);

		if (credentialHeader.vctm) {
			const sdjwtvcMetadataDocument = credentialHeader.vctm
				.map((encodedMetadataDocument: string) =>
