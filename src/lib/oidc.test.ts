...
describe('handleOIDCCallback error paths', () => {
	...
	it('throws when the IdP returns an error parameter', async () => {
		...
		Object.defineProperty(window, 'location', {
			value: new URL(
				'https://app.example.com/cb?error=access_denied&error_description=User+denied+access',
			),
			writable: true,
			configurable: true,
		});
...
describe('validateIdToken', () => {
	...
	it('throws when nonce does not match', async () => {
		...
		await expect(validateIdToken(token, config, 'wrong-nonce', jwksUri)).rejects.toThrow(
