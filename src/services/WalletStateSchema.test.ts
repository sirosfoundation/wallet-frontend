...
describe('The current WalletStateSchema version', () => {
	it("should successfully apply 'new_credential' events on empty baseState", async () => {
		let container: CurrentSchema.WalletStateContainer =
			CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await addNewCredentialEvent(container, 'cred1', '', '');
		const s1 = CurrentSchema.WalletStateOperations.walletStateReducer(
			container.S,
			container.events[0],
		);
	...
	it("should successfully apply 'delete_credential' event on a baseState that includes credentials", async () => {
		let container: CurrentSchema.WalletStateContainer =
			CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await addNewCredentialEvent(container, 'cred1', '', '');
		container.S = CurrentSchema.WalletStateOperations.walletStateReducer(
			container.S,
			container.events[0],
		);
	...
	it('should successfully find the correct point of divergence between two event histories and merge them', async () => {
		...
		const expectMergedEvent2_4 = await CurrentSchema.WalletStateOperations.reparent(
			container2.events[4],
			last(container1.events),
		);
		const expectMergedEvent2_5 = await CurrentSchema.WalletStateOperations.reparent(
			container2.events[5],
			expectMergedEvent2_4,
		);
		const expectMergedEvent2_3 = await CurrentSchema.WalletStateOperations.reparent(
			container2.events[3],
	...
	it('mergeEventHistories de-duplicates new_credential events by credentialId.', async () => {
...

		{
			...
			assert.deepEqual(mergedL, {
				lastEventHash: container.lastEventHash,
				S: container.S,
				events: [...container.events, ...container2.events.slice(1)],
			});
		}

		{
			...
			assert.deepEqual(mergedR, {
				...
				events: [
					...container.events,
					container1.events[1],
					await CurrentSchema.WalletStateOperations.reparent(
						container2.events[1],
						container1.events[1],
					),
				],
	...
	it('mergeEventHistories de-duplicates delete_credential events by credentialId.', async () => {
		...
		last(container2.events).timestampSeconds = 3;
		container2 = await addDeleteCredentialEvent(
			container2,
			(container.events[1] as any).credentialId,
		);
...

		{
			...
			assert.deepEqual(mergedL, {
				lastEventHash: container.lastEventHash,
				S: container.S,
				events: [...container.events, ...container2.events.slice(2)],
			});
		}
	...
	it('mergeEventHistories de-duplicates new_keypair events by kid.', async () => {
		...
		const container1 = await addNewKeypairEvent(container, 'kid1', {
			did: 'did1',
		} as CredentialKeyPair);
		last(container1.events).timestampSeconds = 1;
		let container2 = await addNewKeypairEvent(container, 'kid2', {
			did: 'did2',
		} as CredentialKeyPair);
		last(container2.events).timestampSeconds = 2;
	...
	it('mergeEventHistories de-duplicates delete_keypair events by kid.', async () => {
		...
		const container1 = await addNewKeypairEvent(container, 'kid2', {
			did: 'did2',
		} as CredentialKeyPair);
		last(container1.events).timestampSeconds = 2;
		let container2 = await addNewKeypairEvent(container, 'kid3', {
			did: 'did3',
		} as CredentialKeyPair);
		last(container2.events).timestampSeconds = 3;
	...
	it('mergeEventHistories de-duplicates delete_presentation events by presentationId, and keeps the oldest.', async () => {
		...
		last(container2.events).timestampSeconds = 3;
		container2 = await addDeletePresentationEvent(
			container2,
			(container.events[1] as any).presentationId,
		);
		last(container2.events).timestampSeconds = 4;
	...
	it('mergeEventHistories overwrites conflicting alter_settings events with the latest one.', async () => {
		const defaultSettings: CurrentSchema.WalletStateSettings = {
	...
	it('mergeEventHistories de-duplicates save_credential_issuance_session events by eventId.', async () => {
		...
		const container1 = await addSaveCredentialIssuanceSessionEvent(
			container,
			1,
			'iss1',
			'',
			...
			assert.deepEqual(mergedR, {
				...
				events: [
					...container1a.events,
					{
						...
						parentHash: await CurrentSchema.WalletStateOperations.calculateEventHash(
							last(container1a.events),
						),
					},
				],
			});
	...
	it('should successfully fold one event at a time', async () => {
		...
		container = await addNewCredentialEvent(container, 'cred1', '', '');
		const e1Hash = await CurrentSchema.WalletStateOperations.calculateEventHash(
			container.events[0],
		);
		container = await foldOldEventsIntoBaseState(container, -1);
		container = await addNewCredentialEvent(container, 'cred2', '', '');
		const e2Hash = await CurrentSchema.WalletStateOperations.calculateEventHash(
			container.events[0],
		);
	...
	it('mergeEventHistories maintains the per-branch order of events.', async () => {
		...
		const expectEvent5 = await CurrentSchema.WalletStateOperations.reparent(
			container2.events[3],
			expectEvent4,
		);
		const expectEvent6 = await CurrentSchema.WalletStateOperations.reparent(
			container2.events[4],
			expectEvent5,
		);
		const expectEvent7 = await CurrentSchema.WalletStateOperations.reparent(
			container1.events[4],
			expectEvent6,
		);
		const expectEvent8 = await CurrentSchema.WalletStateOperations.reparent(
			container2.events[5],
			expectEvent7,
		);
		assert.deepEqual(merged, {
	...
	it('mergeEventHistories orders events first in ascending order of schemaVersion.', async () => {
		...
		let container2v1: SchemaV1.WalletStateContainer = jsonParseTaggedBinary(
