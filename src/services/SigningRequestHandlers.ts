...
interface SigningRequestHandlers {
	handleSignJwtPresentation(
		...
		}: { message_id: string; audience: string; nonce: string; verifiableCredentials: any[] },
	): Promise<void>;
	handleGenerateOpenid4vciProofSigningRequest(
		api: BackendApi,
		socket: WebSocket,
		keystore: LocalStorageKeystore,
		{
			...
			nonce,
			issuer,
		}: { message_id: string; audience: string; nonce: string; issuer: string },
	): Promise<void>;
}

export function SigningRequestHandlerService(): SigningRequestHandlers {
	return {
		handleSignJwtPresentation: async (
			socket,
			keystore,
			{ message_id, audience, nonce, verifiableCredentials },
		) => {
			const { vpjwt } = await keystore.signJwtPresentation(nonce, audience, verifiableCredentials);
			logger.debug(
				'VP JWT generated for audience:',
				audience,
				'credentials:',
		...
		handleGenerateOpenid4vciProofSigningRequest: async (
			api: BackendApi,
			socket,
			keystore,
			{ message_id, audience, nonce, issuer },
		) => {
			const [
				{
					proof_jwts: [proof_jwt],
				},
				newPrivateData,
				keystoreCommit,
			] = await keystore.generateOpenid4vciProofs([{ nonce, audience, issuer }]);
