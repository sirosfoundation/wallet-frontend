...
export function useApi(isOnlineProp: boolean = true): BackendApi {
	...
	>> => {
		try {
			...
			if (e?.response?.status === 409 && e?.response?.data?.redirect_tenant) {
				...
				logger.debug("Login: tenant redirect required to:", discoveredTenant);
	...
	const signupWebauthn = useCallback(async (
		...
		try {
			const beginData = retryFrom?.beginData || (await post('/user/register-webauthn-begin', { tenantId: storedTenant, inviteCode })).data;
