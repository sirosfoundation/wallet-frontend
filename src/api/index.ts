...
import {
	...
	toBase64Url,
	transformTaggedResponse,
} from '../util';
import {
	EncryptedContainer,
	makeAssertionPrfExtensionInputs,
	parsePrivateData,
...
import {
	withAuthenticatorAttachmentFromHints,
	withHintsFromAllowCredentials,
} from '@/util-webauthn';
...
export function useApi(isOnlineProp: boolean = true): BackendApi {
	...
	const [sessionState, setSessionState, clearSessionState] = useSessionStorage<SessionState | null>(
		'sessionState',
		null,
	);
	...
	const clearSession = useCallback((): void => {
		authServer
			.logout(tenantId ?? 'default')
			.catch((e) => logger.error('Failed to clear server session', e));
		clearSessionStorage();
		authTokens.clear();
	...
	const updatePrivateDataEtag = useCallback(
	...
	const getWithLocalDbKey = useCallback(
		async (
	...
	const get = useCallback(
		async (
			...
			return getWithLocalDbKey(
				authedRequest.get,
				path,
				sessionState?.uuid || options?.userUuid,
				options,
			);
		},
	...
	const getExternalEntity = useCallback(
		async (
			path: string,
			options?: { headers?: { [header: string]: string } },
			force: boolean = false,
		...
		> => {
			try {
				...
				const getPrivateDataResponse = await get('/user/session/private-data', {
					headers: { 'If-None-Match': getPrivateDataEtag() },
				});
	...
	const updateShowWelcome = useCallback(
		(showWelcome: boolean): void => {
			if (sessionState) {
	...
	const setSession = useCallback(
		async (
			...
			if (isOnline) {
				await fetchInitialData(userRecord.uuid).catch((error) =>
					logger.error('Error in performGetRequests', error),
				);
			}
		},
	...
	const updatePrivateData = useCallback(
		async (newPrivateData: EncryptedContainer): Promise<void> => {
			try {
				async function writeOnIndexedDB() {
					...
					if (!userObject) {
						throw new Error(
							`Could not find user with userHandle ${userHandle} on indexedDB 'users' table`,
						);
					}
				...
				if (
					(e?.response?.status === 412 && (e?.headers ?? {})['x-private-data-etag']) ||
					e.cause === 'x-private-data-etag'
				) {
	...
	const initiatePresentationExchange = useCallback(
		async (verifier_id: number, scope_name: string): Promise<{ redirect_to?: string }> => {
			try {
	...
	const loginWebauthn = useCallback(
		async (
			keystore: LocalStorageKeystore,
			promptForPrfRetry: () => Promise<boolean | AbortSignal>,
			webauthnHints: string[],
	...
	const signupWebauthn = useCallback(
		async (
			name: string,
			keystore: LocalStorageKeystore,
			promptForPrfRetry: () => Promise<boolean | AbortSignal>,
			...
			try {
				const beginData =
					retryFrom?.beginData ||
					(await authServer.registerBegin(storedTenant, inviteCode, oidcIdToken));
				...
				try {
					...
					try {
						const privateData = await keystore.initPrf(credential, prfSalt, promptForPrfRetry, {
							displayName: name,
							userHandle: beginData.createOptions.publicKey.user.id,
						});
						...
						try {
							...
							try {
								updatePrivateDataEtag(await authedRequest.get('/user/session/private-data'));
							} catch (e) {
	...
	const stableUseClearOnClearSession = useMemo(() => {
		return function useClearOnClearSession<T>(
			storageHandle: UseStorageHandle<T>,
		): UseStorageHandle<T> {
...
function useTokenRequest(
	...
	const authHeaders = useCallback(
		async (headers: Record<string, string> = {}): Promise<Record<string, string>> => {
			const tenantId = getTenantFromUrlPath() || 'default';
	...
	const withTokenRejection = useCallback(
		async <T>(send: () => Promise<T>): Promise<T> => {
			const tryRequest = async (send: () => Promise<T>) => {
				try {
					return await send();
				} catch (e) {
					if (e?.response?.status !== 401 || e?.response?.data?.error !== 'Invalid token') throw e;
	...
	const get = useCallback(
		async (path: string, headers: Record<string, string> = {}): Promise<AxiosResponse> =>
			withTokenRejection(async () => {
